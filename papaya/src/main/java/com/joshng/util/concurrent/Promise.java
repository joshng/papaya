...
import static com.joshng.util.concurrent.FunFuture.unwrapExecutionException;
...
public class Promise<T> extends AbstractFunFuture<T> {
    ...
    if (attachFutureCompletion(future)) {
      Futures.addCallback(future, new FutureCallback<T>() {
        ...
        @Override public void onFailure(Throwable t) {
          Exception cause = unwrapExecutionException(t);
  ...
  protected final boolean fail(Throwable failure) {
    Throwable cause = unwrapExecutionException(failure);
