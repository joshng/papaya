...
public class MoreMaps {
  private static final Source MAP_FACTORY = new Source<Map>() {
    public Map get() {
  ...
  private static final F MULTISET_ENTRY_AS_MAP_ENTRY = new F<Multiset.Entry, Map.Entry>() {
    @Override
    public Map.Entry apply(Multiset.Entry input) {
...
//    }
  ...
  private static F GET_ENTRY_KEY = new F<Map.Entry, Object>() {
    public Object apply(Map.Entry from) {
      return from.getKey();
    }
  };
  private static F GET_ENTRY_VALUE = new F<Map.Entry, Object>() {
    public Object apply(Map.Entry from) {
  ...
  public static <K, V> F<Map<? super K, ? extends V>, V> valueAccessor(final K key) {
    return new F<Map<? super K, ? extends V>, V>() {
      public V apply(Map<? super K, ? extends V> map) {
        return map.get(key);
      }
    };
  }
